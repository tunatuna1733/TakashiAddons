  ...
  register('renderWorld', () => {
    ...
    World.getAllEntitiesOfType(EntityArmorStand.class).forEach((armorStand) => {
      ...
      // blue blazes
        ...
        RenderLib.drawEspBox(armorStand.getX(), armorStand.getY() - 2, armorStand.getZ(), 1, 2, 0.2, 0.2, 1, 1, true);
      }
      // red blazes
      else if (armorStand.getName().removeFormatting().includes('[Lv150] Ashfang Underling')) {
        RenderLib.drawEspBox(armorStand.getX(), armorStand.getY() - 2, armorStand.getZ(), 1, 2, 1, 0.2, 0.2, 1, true);
      }
      // blackhole
      else {
