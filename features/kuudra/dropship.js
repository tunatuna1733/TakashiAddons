...
registerWhen(
  ...
  }).setCriteria("[NPC] Elle: A Dropship is approaching! Take it down before it's too late!"),
  () => settings.dropship && getCurrentArea().includes('Kuudra'),
...
registerWhen(
  register('chat', () => {
    isDropshipApproaching = true;
    dropshipTimer = Date.now();
  }).setCriteria("[NPC] Elle: A fleet of Dropships are approaching! Take them down before it's too late!"),
  () => settings.dropship && getCurrentArea().includes('Kuudra'),
  { type: 'chat', name: moduleName },
);
