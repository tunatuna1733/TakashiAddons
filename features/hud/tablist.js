...
export const addCustomHud = () => {
  ...
  categoryTexts.forEach((c) => {
    ChatLib.chat(new TextComponent(` &e${c}`).setClick('run_command', `/takashiaddcustomtablisthud ${c}`));
  });
};
...
}).setCommandName('takashiaddcustomtablisthud', true);

export const removeCustomHud = () => {
  ChatLib.chat(`${CHAT_PREFIX} &aPlease select the hud you want to &c&lremove&r&a.`);
  customHudsData.data.forEach((h) => {
    ChatLib.chat(new TextComponent(` &e${h.name}`).setClick('run_command', `/takashiremovecustomtablisthud ${h.name}`));
  });
};
...
register('command', (...args) => {
  ...
  loadHuds();
}).setCommandName('takashiremovecustomtablisthud', true);
