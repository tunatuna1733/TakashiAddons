  ...
  try {
    ...
    if (areaLine) {
      if (areaLine.includes('Dungeon: ') || areaLine.includes('Kuudra')) {
        setTimeout(() => {
          const zoneLine = Scoreboard.getLines().find(
            (line) => line.getName().includes('⏣') || line.getName().includes('ф'),
          );
          if (zoneLine) {
            const zone = zoneLine
              .getName()
      ...
      if (retryCount < 20) {
        ...
        if (isInSkyblock()) ChatLib.chat(`${CHAT_PREFIX} &c[ERROR] Failed to get current area :(`);
      }
    }
  } catch (e) {
    console.log('[Takashi] Error in area.js');
    console.log(e);
  }
};
